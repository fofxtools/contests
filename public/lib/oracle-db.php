...
/*
 * lib/oracle-db.php — read-only connection to a static snapshot of the Oracle
 * Challenge contest DB (oraclechallenge.com), frozen since 2020. Mirrors
 * lib/db.php. Feature code that uses this lives in lib/paa.php (Points Above
 * Average) and lib/oracle-functions.php (everything else Oracle-related —
 * kept in one file by design, not split per page).
 *
 * Snapshot lives in data/tables/oracle.sqlite, generated by
 * scripts/mysql-to-sqlite-dump.php from .private-config.php's 'db_oracle'
 * creds (dev-only; not read at request time — the source data never
 * changes, so this never needs to run again unless the snapshot itself
 * needs updating).
 */
...
/** Shared read-only PDO handle to the Oracle Challenge DB snapshot. */
function oracle_db(): PDO
{
    static $db = null;
    if ($db === null) {
        $dsn = 'sqlite:' . dirname(__DIR__, 2) . '/data/tables/oracle.sqlite';
        $db  = new PDO($dsn, null, null, [
            PDO::ATTR_ERRMODE            => PDO::ERRMODE_EXCEPTION,
            PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
        ]);
        // Users.Name/Teams.Name were latin1_swedish_ci in MySQL, which sorts
        // '_' after every letter (unlike SQLite, where '_' sorts between
        // upper- and lowercase). Matches MySQL order exactly for all 717
        // Users + 368 Teams names — verified directly against live data.
        $db->sqliteCreateCollation('MYSQLCI', function (string $a, string $b): int {
            $norm = fn (string $s): string => str_replace('_', "\x7F", strtolower($s));
