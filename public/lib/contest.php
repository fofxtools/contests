...
function mysql2timestamp(string $datetime): int {
    ...
    return (int) mktime((int)$time[0], (int)($time[1] ?? 0), (int)($time[2] ?? 0),
                        (int)($date[1] ?? 1), (int)($date[2] ?? 1), (int)($date[0] ?? 1970));
}
...
function displaybracket($contest, $title, $numrounds = 6, $numdivisions = 4, $outputstyle = ""): void {
    if (!preg_match('/^[a-z0-9_]+$/', (string)$contest)) { echo '<p>Unknown bracket.</p>'; return; }
    ...
    for ($j = 0; $j <= $numrounds; $j++) {
        ...
        for ($i = 0; $i < $numwinners; $i++) {
            if ($j == 0) { $winnersmatrix[$j][$i] = $i; continue; }
            if (($winnersmatrix[$j-1][$i*2] ?? -1) == -1 || ($winnersmatrix[$j-1][$i*2+1] ?? -1) == -1) {
                ...
                if ($votes1 == $votes2)      $winnersmatrix[$j][$i] = -1;
                elseif ($votes1 > $votes2)   $winnersmatrix[$j][$i] = $a;
                else                         $winnersmatrix[$j][$i] = $b;
            }
        }
    }
    ...
    for ($i = 0; $i < $numrows; $i++) {
        ...
        if ($i % ($entrantsperdivision * 2) == 0) echo "<td rowspan=$numrowsperdivision class='division'>&nbsp;</td>";
        if (($i + 1) % ($entrantsperdivision * 2) == 0) echo "<td></td>";
        for ($j = 0; $j <= $numrounds; $j++) {
            $wm = $winnersmatrix[$j][floor($i / pow(2, $j + 1))] ?? -1;
            if ($wm != -1) {
                $entrant = $rows[$wm]['entrant'] ?? '_____';
                $jplus1  = $j + 1;
                if ($j < $numrounds && $displayvotes != -1) $votes = $rows[$wm]["round{$jplus1}votes"] ?? '';
                else $votes = "";
                if ($votes == 0) $votes = "";
                $seed = $rows[$wm]['seed'] ?? '';
                if ($seed !== '' && $seed < 10) $seed = "&nbsp;&nbsp;" . $seed;
                $link = $rows[$wm]['link'] ?? '';
                if ($link == "") $link = 'http://images.google.com/images?q=' . str_replace(' ', '%20', (string)$entrant);
            } else {
                $entrant = "_____"; $votes = ""; $seed = ""; $link = "";
            }
...
function displaymatches($contest, $node): void {
    ...
    if (!in_array($fields, ['', '1', '2', 'all'], true)) $fields = '';   // whitelist: kills reflected-XSS via ?fields=
    $sort   = $_GET['sort']   ?? 'pollid';
    $type   = strtoupper((string)($_GET['type'] ?? 'DESC'));
    if (!in_array($sort, $SORTABLE, true)) $sort = 'pollid';
    if ($type !== 'ASC' && $type !== 'DESC') $type = 'DESC';
    $node = (int)$node;
    ...
    for ($i = 0; $i < $num; $i++) {
        ...
        if ($r['winneroracleaverage'] == 0 || $woa === '-100.00%') $woa = 'N/A';
        ...
        $pc1 = gf_pct($r['percent1']); $pc2 = gf_pct($r['percent2']); $pcd = gf_pct($r['percentdifference']);
...
function listmatches($contest, $num_entrants = 2): void {
    ...
    for ($i = 1; $i <= $num_entrants; $i++) echo "<th>Entrant $i</th>";
    echo "</tr>\n";
    foreach ($rows as $row) {
        $m = (int)$row['matchnum'];
        echo "<tr>";
        ...
        for ($i = 1; $i <= $num_entrants; $i++) echo "<td>" . htmlspecialchars((string)($row["entrant$i"] ?? '')) . "</td>";
        echo "</tr>\n";
    }
    echo "</table>";
}
...
function displayupdates($matchnum, $sort = "totalvotes", $type = "DESC", $num_entrants = 2): void {
    ...
    if (!in_array($sort, $SORTABLE, true)) $sort = 'totalvotes';
    if ($type !== 'ASC' && $type !== 'DESC') $type = 'DESC';
    if ($matchnum == '2084') $sort = 'time';                 // Kefka/Vercetti fix (from original)
    ...
    if ($num_entrants == 2) {
        ...
        $pcts = [];
        for ($i = 1; $i <= $num_entrants; $i++) $pcts[] = "votes$i/(votes1+votes2+votes3+votes4) AS percent$i";
    ...
    if ($num_entrants > 2) {
        $latest_row = 0; $latest_time = 0;
        for ($i = 0; $i < $numrows; $i++) {
            $t = mysql2timestamp((string)$result_array[$i]['time']);
            if ($t > $latest_time) { $latest_time = $t; $latest_row = $i; }
        }
        $x_array = [];
        ...
        foreach ($x_array as $name => $votes) {
            if ($name != '') $output .= "<tr><td>$name</td><td>" . sprintf("%0.2f", round($votes / ($strongest + $votes) * 100, 2)) . "%</td></tr>\n";
        }
        $output .= "</table>\n<br />\n";
    }
    ...
    $r0 = $result_array[0];
    $e1 = $r0['entrant1']; $e2 = $r0['entrant2'];
    $e3 = $r0['entrant3'] ?? ''; $e4 = $r0['entrant4'] ?? '';
    ...
    if ($num_entrants > 2) $output .= "<th $th><a href='$base&sort=percent3$sfx' title='Sort $type by {$e3}&#039;s percentage'>{$e3}'s %</a></th>";
    if ($num_entrants > 3) $output .= "<th $th><a href='$base&sort=percent4$sfx' title='Sort $type by {$e4}&#039;s percentage'>{$e4}'s %</a></th>";
    if ($num_entrants == 2) {
        $output .= "<th><a href='$base&sort=lead$sfx' title='Sort $type by the amount of the Lead'>Lead</a></th>";
    } else {
        $output .= "<th>Lead 1</th><th>Lead 2</th>";
        if ($num_entrants > 3) $output .= "<th>Lead 3</th>";
    }
    if ($sort == "time" && $type == "ASC") {
        $output .= "<th>Update % A</th><th>Update % B</th>";
        if ($num_entrants > 2) $output .= "<th>Update % C</th>";
        if ($num_entrants > 3) $output .= "<th>Update % D</th>";
        $output .= "<th>Update Votes A</th><th>Update Votes B</th>";
        if ($num_entrants > 2) $output .= "<th>Update Votes C</th>";
        if ($num_entrants > 3) $output .= "<th>Update Votes D</th>";
    }
    ...
    if ($num_entrants > 2) $output .= "<th $th><a href='$base&sort=votes3$sfx' title='Sort $type by {$e3}&#039;s votes'>{$e3}</a></th>";
    if ($num_entrants > 3) $output .= "<th $th><a href='$base&sort=votes4$sfx' title='Sort $type by {$e4}&#039;s votes'>{$e4}</a></th>";
    ...
    for ($i = 0; $i < $numrows; $i++) {
        $row = $result_array[$i];
        $v1 = (int)$row['votes1']; $v2 = (int)$row['votes2'];
        $v3 = $num_entrants > 2 ? (int)$row['votes3'] : 0;
        $v4 = $num_entrants > 3 ? (int)$row['votes4'] : 0;
        $tv = $v1 + $v2 + $v3 + $v4;
        if ($tv > 0) {
            $pc1 = number_format($v1 / $tv * 100, 2) . "%"; $pc2 = number_format($v2 / $tv * 100, 2) . "%";
            $pc3 = number_format($v3 / $tv * 100, 2) . "%"; $pc4 = number_format($v4 / $tv * 100, 2) . "%";
        } else { $pc1 = $pc2 = $pc3 = $pc4 = "0%"; }

        if ($num_entrants == 2) {
            $lead = $row['lead'];
        } else {
            $va = [$v1, $v2, $v3, $v4]; rsort($va, SORT_NUMERIC);
            $lead1 = $va[0] - $va[1]; $lead2 = $va[1] - $va[2];
            if ($num_entrants > 3) $lead3 = $va[2] - $va[3];
        }

        if ($sort == "time" && $type == "ASC") {
            if ($i == 0) { $lu1 = $v1; $lu2 = $v2; $lu3 = $v3; $lu4 = $v4; }
            else { $lu1 = $v1 - $lastvotes1; $lu2 = $v2 - $lastvotes2; $lu3 = $v3 - $lastvotes3; $lu4 = $v4 - $lastvotes4; }
            $tlu = $lu1 + $lu2 + $lu3 + $lu4;
            if ($tlu > 0) {
                $lp1 = number_format($lu1 / $tlu * 100, 2) . "%"; $lp2 = number_format($lu2 / $tlu * 100, 2) . "%";
                $lp3 = number_format($lu3 / $tlu * 100, 2) . "%"; $lp4 = number_format($lu4 / $tlu * 100, 2) . "%";
            } else {
                $lp1 = $lp2 = $lp3 = $lp4 = number_format(100 / $num_entrants, 2) . "%";
            }
            $lastvotes1 = $v1; $lastvotes2 = $v2; $lastvotes3 = $v3; $lastvotes4 = $v4;
        }
        ...
        if ($num_entrants > 2) $output .= "<td><div style='text-align: right;'>$pc3</div></td>";
        if ($num_entrants > 3) $output .= "<td><div style='text-align: right;'>$pc4</div></td>";
        if ($num_entrants == 2) {
            ...
            if ($num_entrants > 3) $output .= "<td><div style='text-align: right;'>$lead3</div></td>";
        }
        if ($sort == "time" && $type == "ASC") {
            $output .= "<td><div style='text-align: right;'><b>$lp1</b></div></td><td><div style='text-align: right;'><b>$lp2</b></div></td>";
            if ($num_entrants > 2) $output .= "<td><div style='text-align: right;'><b>$lp3</b></div></td>";
            if ($num_entrants > 3) $output .= "<td><div style='text-align: right;'><b>$lp4</b></div></td>";
            $output .= "<td><div style='text-align: right;'>$lu1</div></td><td><div style='text-align: right;'>$lu2</div></td>";
            if ($num_entrants > 2) $output .= "<td><div style='text-align: right;'>$lu3</div></td>";
            if ($num_entrants > 3) $output .= "<td><div style='text-align: right;'>$lu4</div></td>";
        }
        $output .= "<td><div style='text-align: right;'>$v1</div></td><td><div style='text-align: right;'>$v2</div></td>";
        if ($num_entrants > 2) $output .= "<td><div style='text-align: right;'>$v3</div></td>";
        if ($num_entrants > 3) $output .= "<td><div style='text-align: right;'>$v4</div></td>";
...
function amr_rows(): array {
    ...
    foreach (gfq($sql) as $u) {
        $mn = (int)$u['matchnum'];
        if (isset(AMR_SKIP[$mn]) || in_array((string)$u['contest'], AMR_PRE2007, true)) continue;
        $ents = [];
        for ($i = 1; $i <= 4; $i++) {
        ...
        if (count($ents) < 2 || array_sum(array_column($ents,'votes')) === 0) continue;  // degenerate final row
...
function amr_canon(string $name): string { return AMR_ALIAS[$name] ?? $name; }
...
function amr_mk(int $poll, string $ccode, string $date, array $ents): array {
    ...
    foreach ($ents as &$e) $e['pct'] = $total > 0 ? $e['votes'] / $total * 100 : 0.0;
    unset($e);
...
function all_match_results(): void {
    ...
    $rows     = amr_rows();
    $allCount = count($rows);
    if ($fEnt !== '') $rows = array_values(array_filter($rows, fn($r) => in_array($fEntC, $r['canon'], true)));
    if ($fCon !== '') $rows = array_values(array_filter($rows, fn($r) => $r['cname'] === $fCon));

    usort($rows, $S[$sort]);
    if ($dir === 'desc') $rows = array_reverse($rows);

    $nM = 0; $byNe = [2=>0,3=>0,4=>0];
    foreach ($rows as $r) { if ($r['poll'] <= 2566) $nM++; $byNe[$r['ne']]++; }
    $nU = count($rows) - $nM;
...
<?php endif; ?>
<?php $i = 0; foreach ($rows as $r): $i++; $fl = AMR_FLAG[$r['poll']] ?? null; ?>
<tr>
 <td class="amr-n"><?= $i ?></td>
...
<?php
$aliasGroups = [];
foreach (AMR_ALIAS as $aliasVariant => $aliasCanon) $aliasGroups[$aliasCanon][] = $aliasVariant;
