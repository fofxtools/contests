...
if (!function_exists('microtime_float')) {
    function microtime_float(): float { return microtime(true); }
}
...
if (!function_exists('script_timer')) {
    ...
    {
        global $timer_array;
        if (!is_array($timer_array)) $timer_array = [];
        if (strtolower($start_or_end) !== 'end') {
            $timer_array[$script_name]['Start'] = microtime_float();
        } else {
            $timer_array[$script_name]['End'] = microtime_float();
            $timer_array[$script_name]['Elapsed'] =
                round($timer_array[$script_name]['End'] - $timer_array[$script_name]['Start'], 5);
        }
    }
}
...
if (!function_exists('get_extrapolated_results')) {
    ...
    function get_extrapolated_results(array $strength_set): array
    {
        ...
        if ($peak <= 0) return array_fill(0, count($strength_set), 0.0);
        $scalar = 1 / $peak;
        for ($i = 0; $i < count($strength_set); $i++) {
        ...
        for ($char_index = 0; $char_index < $num_chars; $char_index++) {
            ...
            foreach ($strength_set as $n) $char_prob *= $n;

            $strength_set_others = [];
            for ($index = 0; $index < $num_chars; $index++) {
                if ($index !== $char_index) $strength_set_others[] = $strength_set[$index];
            }
            ...
            for ($num_inv = 1; $num_inv <= $num_other_chars; $num_inv++) {
                ...
                foreach ($combinations as $combin) {
                    ...
                    foreach ($inverted_array as $n) $term_prob *= $n;
                    $char_prob += $term_prob;
                }
            }
...
if (!function_exists('get_extrapolated_table')) {
    ...
    function get_extrapolated_table(array $input_values, $input_names = false): string
    {
        ...
        for ($i = 0; $i < count($value_array); $i++) {
            ...
            if (is_array($input_names) && ($input_names[$i] ?? '') !== '') $name = $input_names[$i];
            else $name = "Character $char";
